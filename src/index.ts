...
server.tool(
  "generate_image",
  "Generate image via OpenAI-compatible POST /v1/images/generations. Supports text-to-image and image-to-image (pass `image`). Returns image URL or base64.",
  {
  ...
  async (args) => {
    ...
    if (args.extra !== undefined) Object.assign(body, args.extra);
    ...
    const data = await jsonOrThrow<{
      created?: number;
      data?: Array<{ url?: string; b64_json?: string; seed?: number; finish_reason?: string }>;
      error?: { message?: string };
    }>(resp);

    if (data.error) throw new Error(`image error: ${data.error.message}`);
    const item = data.data?.[0];
    if (!item) throw new Error("image: empty data response");
...
server.tool(
  "generate_speech",
  "Text-to-speech via OpenAI-compatible POST /v1/audio/speech. Returns audio (base64) or URL. Voice & model default from SYNTHFLUX_TTS_VOICE / SYNTHFLUX_TTS_MODEL env.",
  {
  ...
  async (args) => {
    ...
    if (args.sample_rate !== undefined) body.sample_rate = args.sample_rate;
    ...
    if (contentType.includes("application/json")) {
      ...
      if (!data.url) throw new Error("tts: json response without url");
...
server.tool(
  "create_video_task",
  "Create async video task via POST /v1/videos (Agnes-style). Supports text-to-video, image-to-video, keyframes. Returns video_id to poll via get_video_status.",
  {
  ...
  async (args) => {
    ...
    if (args.extra_body !== undefined) body.extra_body = args.extra_body;
    ...
    if (data.error) throw new Error(`video task error: ${data.error.message}`);
    return {
      content: [{
        type: "text",
        text: JSON.stringify({
...
server.tool(
  "get_video_status",
  "Poll Agnes-style video task by video_id. Returns progress and final url when status=='completed'.",
  {
  ...
  async (args) => {
    ...
    if (args.model_name) params.set("model_name", args.model_name);
    ...
    if (data.error) throw new Error(`video failed: ${JSON.stringify(data.error)}`);
    return {
      content: [{
        type: "text",
        text: JSON.stringify({
...
server.tool(
  "generate_video",
  "One-shot helper: create video task, poll until status=='completed' or 'failed', return final url. Blocking; 30-180s typical depending on num_frames.",
  {
    prompt: z.string(),
  ...
  async (args) => {
    ...
    if (args.image !== undefined) createBody.image = args.image;
    if (args.negative_prompt !== undefined) createBody.negative_prompt = args.negative_prompt;
    if (args.extra_body !== undefined) createBody.extra_body = args.extra_body;

    const createResp = await mediaFetch("/v1/videos", {
      method: "POST",
      body: JSON.stringify(createBody),
    });
    const task = await jsonOrThrow<{ video_id?: string; error?: { message?: string } }>(createResp);
